...
import { Encoding, Frame, Header, ZACK_HEADER, ZDATA_HEADER, ZEOF_HEADER, ZFIN_HEADER, ZRPOS_HEADER, ZRQINIT_HEADER, decodeHeader, createZrinit, writeSliceEscaped, Zrinit } from './header.js'
...
export enum SenderEvent {
  ...
  /** File skipped by receiver (e.g. duplicate filename) */
...
export class Sender {
  ...
  private queueZfile (): void {
    ...
    fileInfo.push(...encodeUtf8(this.fileName))
  ...
  private onZskip (): void {
    // Receiver chose to skip the current file (e.g. file already exists and
    // its policy refuses to overwrite). Treat it as a successful end-of-file
    // for the current file so the upper layer can move on or call
    // finishSession() instead of waiting on ZRPOS forever.
    switch (this.state) {
      case SendState.WaitFilePos:
      case SendState.NeedFileData:
      case SendState.WaitFileAck:
      case SendState.WaitFileDone:
        this.pendingEvent = SenderEvent.FileSkipped
        this.hasFile = false
        this.pendingRequest = null
        this.frameRemaining = 0
        this.frameNeedsHeader = false
...
export class Receiver {
  ...
  private parseZfileBuf (): void {
    ...
    this.fileName = decodeUtf8(fields[0])
